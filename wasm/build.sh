#!/usr/bin/env bash
# Build the browser (`worker:`) build of the pprof VGI worker: pprof-wasm
...
LIB="target/$TARGET/release/libpprof_wasm.a"
...
#
# OPT_LEVEL=3 overrides the workspace's release profile for this target only; it
# does not change the native build.
#
...
# which the Arrow encoders + prost decode nest too deeply for; an overflow
# surfaces as a bare "memory access out of bounds". INITIAL_MEMORY must cover the
# main stack + every pthread stack up front, or startup stalls growing shared
# memory while the pool spawns.
echo "==> emcc link"

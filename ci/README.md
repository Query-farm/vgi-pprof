# CI: the vgi-pprof worker integration suite
...
(`test/sql/*.test`) against the vgi-pprof VGI worker through the **real DuckDB
...
| `TRANSPORT`  | `VGI_PPROF_WORKER` (the ATTACH LOCATION) | how the worker is launched |
|--------------|------------------------------------------|----------------------------|
| `subprocess` | `…/target/release/pprof-worker`          | DuckDB spawns the stdio binary (default) |
| `http`       | `http://127.0.0.1:<port>`                | `pprof-worker --http` (auto port; prints `PORT:<n>` on stdout, which the script polls for) |
| `unix`       | `unix:///tmp/pprof.<pid>.sock`           | `pprof-worker --unix <sock>` (prints `UNIX:<sock>` on stdout + creates the socket; the script waits for both) |
...
1. **Build the worker** — `cargo build --release --bin pprof-worker`. The
   compiled `target/release/pprof-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_PPROF_WORKER}`).
...
3. **Preprocess** — the standalone runner links none of the extensions the
   ...
   through untouched. (The vgi-pprof `.test` files already `LOAD vgi;`
   explicitly and use `require-env VGI_PPROF_WORKER`, so the `require`-rewrite is
...
4. **Run** — [`run-integration.sh`](run-integration.sh) brings up the worker for
   the selected `TRANSPORT` and sets `VGI_PPROF_WORKER` accordingly, stages the
...
WORKER_BIN="$PWD/target/release/pprof-worker" \
TRANSPORT=subprocess \
  ci/run-integration.sh

# HTTP leg: launches `pprof-worker --http`, reads PORT:<n>, attaches http://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/pprof-worker" \
  TRANSPORT=http ci/run-integration.sh

# Unix leg: launches `pprof-worker --unix /tmp/pprof.<pid>.sock`, attaches unix://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/pprof-worker" \

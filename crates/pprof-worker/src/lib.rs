...
//! the `pprof-wasm` crate (the browser build, which serves the same `Worker`
...
/// The catalog name DuckDB sees in `ATTACH 'pprof' (TYPE vgi, …)`. Defaults to
/// `pprof`, but honors an explicit override so a test harness can rename it.
...
/// Build a fully-registered worker: every table function plus the catalog

...
#
# Single image that serves every network transport of the `pprof` VGI worker:
...
#
# Like vgi-units this worker is STATELESS: it decodes a pprof profile handed to
# it (a path, a glob, a list of paths, or inline bytes) into SQL rows and keeps
# nothing between attaches, so there is no /data volume, no registry, and no
# `farm.query.vgi.volumes` mount-discovery label. The image is just the binary +
# a tiny entrypoint. (To decode on-disk profiles through a *container* worker,
# bind-mount the directory in and pass in-container paths.)
# syntax=docker/dockerfile:1

# ---- build stage -----------------------------------------------------------
# Pinned glibc (bookworm) so the binary links against the same libc the slim
# runtime ships. The pprof-core build.rs runs prost-build codegen against the
# vendored proto/profile.proto; `protoc` is supplied hermetically by the
# `protoc-bin-vendored` crate, so no system protoc is needed in this stage.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin pprof-worker \
    && cp target/release/pprof-worker /pprof-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-pprof
...
LABEL org.opencontainers.image.title="vgi-pprof" \
      org.opencontainers.image.description="Decode pprof (profile.proto) CPU/heap/alloc/mutex profiles into SQL rows as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_PPROF_GIT_COMMIT=${GIT_COMMIT}
...
# rewrite the whole ~16MB binary into a second layer (overlayfs copies the file
# on a metadata change), needlessly doubling its on-disk footprint in the image.
COPY --from=build --chmod=0755 /pprof-worker /usr/local/bin/pprof-worker
